...
//	pthread_t rtd_thread;
	...
	qusb_log("Test test test test test ");
	qusb_log("est est est ");
	qusb_log("tes tes tes %i foo",3);
	...
	qusb_log("Found %i ports, init...",portl.portc);
...
//		printf("postmmap");fflush(stdout);
	...
	for (int i = 0; i < o.mport; i++) {
		...
		if (ret) {
			qusb_log("Thread %i failed!: %i.\n", i, ret); exit(EEPP_THREAD);
			printe("Thread %i failed!: %i.\n", i, ret); exit(EEPP_THREAD);
		} else active_threads++;
	}

	qusb_log("inititalization done.\n");
	printf("done.\n"); fflush(stdout);
		...
		for (int i = 0; i < o.mport; i++) {
			...
			if (ret == 0) {
				...
				if (running) {
					...
					if (ret) {
						qusb_log("Port %i revive failed!: %i.\n", i, ret); exit(EEPP_THREAD);
...
void *qusb_data_pt(void *threadarg) {
	...
	char *dataz, ostr[1024];
	struct tm ct;
    struct timeval start, now, then;
//    bool fifo_acqseq = false;
	FILE *ofile;
//    short int *hptr;
    ...
    printf("Module %i data thread init.\n", arg.np); fflush(stdout);
...
//	printf("size of struct: %li, sync: %li\n", sizeof(struct frame_sync), sizeof(sync));
    ...
    qusb_log("Module %i data acquisition start.  Output file: '%s'.  Acqsize: %i.\n", arg.np, ostr, arg.o.acqsize);
    if (arg.o.debug) { printf("Module %i data acquisition start.  acqsize = %i\n", arg.np, arg.o.acqsize); fflush(stdout); }
			...
			if ((arg.o.dt > 0) && (size_commit == arg.o.acqsize)) {
				...
				if (arg.o.debug)
					printf("port %i rtd moving rtdbytes %i from cfb %p to rtdb %p with %lu avail.\n",
						arg.np, rtdbytes, dataz, arg.rtdframe, size_commit);
